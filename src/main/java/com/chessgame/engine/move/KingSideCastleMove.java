...
public class KingSideCastleMove extends Move {
	...
	public KingSideCastleMove(King oldKing, Board board) {
		super(board);
		...
		return oldKing.getAlliance() == Alliance.WHITE ? Point.of(6, 0) : Point.of(6, 7);
        }
	...
	public Board execute(Board board) {
		...
		if (oldKing.getAlliance() == Alliance.WHITE) {
			...
			oldKingsRook = board.getPiece(Point.of(7, 0)).orElseThrow();
			newKingsRook = oldKingsRook.withPosition(Point.of(5, 0));
		} else {
			newKing = oldKing.withPosition(this.getDestination());
			oldKingsRook = board.getPiece(Point.of(7, 7)).orElseThrow();
			newKingsRook = oldKingsRook.withPosition(Point.of(5, 7));
		}
